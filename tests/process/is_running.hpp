#ifndef PROCESS_IS_RUNNING_HPP
#define PROCESS_IS_RUNNING_HPP
...
namespace process {
    namespace details {
        ...
        // This bit arrangement of status codes is not guaranteed by POSIX, but (according to
        // comments in the glibc <bits/waitstatus.h> header) is the same across systems in practice.
        ...
        inline bool is_running(const child_handle &p, int &exit_code,
            ...
            if (ret == -1) {
                // because it no child is running, than this one isn't either,
                // obviously.
                if (errno != ECHILD)
                    ec = get_last_error();
                return false;
        ...
        inline bool is_running(const child_handle &p, int &exit_code) {
            ...
            throw_error(ec, "waitpid(2) failed in is_running");
            return b;
        }
